...
public class ResponseCacheService : IResponseCacheService {
    ...
    public async Task CacheResponseAsync(string cacheKey, object? response, TimeSpan timeTimeLive) {
        ...
        await _distributedCache.SetStringAsync(cacheKey.ToLower(), serializedResponse, new DistributedCacheEntryOptions {
            AbsoluteExpirationRelativeToNow = timeTimeLive
        });
    }

    public async Task<string> GetCachedResponseAsync(string cacheKey) {
        var cachedResponse = await _distributedCache.GetStringAsync(cacheKey.ToLower());
